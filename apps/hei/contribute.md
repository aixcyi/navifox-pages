...
　　非常感谢对《蓝溪拾遗》感兴趣！本文档描述了如何在《蓝溪拾遗》项目下添加、修改妖精与人类（以下简称妖灵）、会馆等的信息。

## 快速开始

1. 注册或登录到 GitHub 并前往源代码[仓库](https://github.com/aixcyi/navifox-pages)。
2. 将整个仓库[分叉](https://github.com/aixcyi/navifox-pages/fork)（Fork）到你的账号中。
3. 克隆你的仓库到本地。
4. 新建并切换到一条分支，建议以 `hei` 为分组名，例如 `hei/feat-new-guild` 对应新建会馆。
5. 根据仓库根目录下的 README.md 安装并配置环境。
6. 修改需要的位置，或添加必要的文件。
7. 在仓库根目录下运行 `pnpm run dev:hei`然后到浏览器访问 `http://localhost:5173` 即可预览《蓝溪拾遗》。
8. 重复前两步，直到认为修改完毕。
9. 在仓库根目录下运行 `pnpm run format` 格式化代码。
10. 在仓库根目录下运行 `pnpm run build:hei` 确认构建无误。
11. 提交修改，然后推送分支到你的仓库。
12. 前往 GitHub 创建[推送请求](https://github.com/aixcyi/navifox-pages/pulls)（Pull Request）。
13. 前往[罗狐会馆](/guild/foxery)追踪状态~~（在线催更）~~。
